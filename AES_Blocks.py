from simbolic_calculation import fxmul, fxmod

def AES_Modification(modify,a,b):
	sbox=[[0x63,0x7c,0x77,0x7b,0xf2,0x6b,0x6f,0xc5,0x30,0x1,0x67,0x2b,0xfe,0xd7,0xab,0x76],
...
def ARK(key,d):
	for i in range(16):
		d[i%4][i//4]=(d[i%4][i//4])^key[i]
	return d

def KS(key,i,sbox):
	key_result=[None for i in range(16)]
	TWi_1=[None for i in range(4)]
	shift=[None for i in range(4)]
	S_BOX_alternate=shift
	ri=fxmul(2**(i-1),1)
	...
	shift[3]=key[12]
	for n in range(4):
		S_BOX_alternate[n]=sbox[shift[n]//0x10][shift[n]%0x10]
	TWi_1[0]=ri^S_BOX_alternate[0]
	for i in range (1,4):
		TWi_1[i]=S_BOX_alternate[i]
	for n in range(4):
		key_result[n]=TWi_1[n]^key[n]
	for n in range(4,16):
		key_result[n]=key_result[n-4]^key[n]
	return key_result

def BS(plaintext,sbox):
	b=[[None for i in range(4)] for j in range(4)]
	for i in range(4):
		for j in range(4):
			row=(plaintext[i][j])//0x10
			col=(plaintext[i][j])%0x10
			b[i][j]=sbox[row][col]
	return b
...
def MC(c):
	...
	d=[[None for i in range(4)] for j in range(4)]
	for i in range(4):
		for j in range(4):
			d[i][j]=((fxmul(M[i][0],c[0][j])^fxmul(M[i][1],c[1][j]))^(fxmul(M[i][2],c[2][j])))^(fxmul(M[i][3],c[3][j]))
	return d